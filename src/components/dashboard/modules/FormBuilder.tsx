...
/** Domyślne klocki, które użytkownik może dodać do formularza jednym kliknięciem. */
const PRESETS: Omit<FormField, "id">[] = [
  { type: "text", label: "Imię i nazwisko", name: "name", placeholder: "Jan Kowalski", required: true },
  { type: "email", label: "Email", name: "email", placeholder: "jan@przyklad.pl", required: true },
  { type: "tel", label: "Telefon", name: "phone", placeholder: "+41 79 123 45 67" },
  { type: "textarea", label: "Wiadomość", name: "message", placeholder: "Treść wiadomości…", required: true },
];
    ...
    <section>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          ...
          <h3 className="mt-6 text-sm font-semibold text-slate-900">Dodaj klocek</h3>
          <div className="mt-2 flex flex-wrap gap-2">
            {PRESETS.map((p) => (
              <button key={p.name} onClick={() => addPreset(p)} className="btn-secondary text-sm py-2 px-3">
            ...
            >
              <Plus size={14} /> Własne pole
            </button>
          </div>

          <h3 className="mt-6 text-sm font-semibold text-slate-900">Pola ({fields.length})</h3>
          <ul className="mt-2 space-y-2">
            {fields.map((f) => (
              <li key={f.id} className="rounded-xl border border-slate-200 p-3">
                ...
                <input value={f.placeholder ?? ""} onChange={(e) => updateField(f.id, { placeholder: e.target.value })} placeholder="Placeholder" className="input py-2 text-sm mt-2" />
                <label className="mt-2 flex items-center gap-2 text-xs text-slate-600">
                  <input type="checkbox" checked={!!f.required} onChange={(e) => updateField(f.id, { required: e.target.checked })} /> Pole wymagane
                </label>
              </li>
            ))}
            {fields.length === 0 && <p className="text-sm text-slate-400 text-center py-6">Brak pól – dodaj klocek powyżej.</p>}
          </ul>
        </div>
